...
def get_video_info(url):
    ...
    ydl_opts = {
        ...
        'no_warnings': True,
        'simulate': True,
        'format': 'best',  # Try to get the best available format
        'format_sort': ['res:1080', 'res:720'],  # Fallback options
    }
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            return {
                ...
                'url': url,
                'size': best_format.get('filesize', 0) if best_format else 0,
                'resolution': best_format.get('resolution', 'unknown') if best_format else 'unknown',
                'format_id': best_format.get('format_id', 'unknown') if best_format else 'unknown'
            }
